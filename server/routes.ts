...
export async function registerRoutes(
  ...
  app.get("/api/bounty-feed", rateLimit(30, 60000), async (req, res) => {
    try {
      const feedUrls = [
        { id: 'h1', name: 'HackerOne', url: 'https://hackerone.com/hacktivity.rss', category: 'bug-bounty' },
      ...
      for (const feed of feedUrls) {
        try {
          ...
          console.error(`[RSS] Failed to parse feed ${feed.name}:`, err.message);
        }
      }
    ...
    try {
      ...
      try {
        ...
        if (cisaRes.ok) {
          ...
          cisaItems = (cisaData.vulnerabilities || []).slice(0, 20);
        }
      } catch (e: any) {
      ...
      res.json({
        success: true,
        feeds: {
          cisa: { name: 'CISA KEV', items: cisaItems },
          urlhaus: { name: 'URLHaus', items: [
  ...
  app.get("/api/business-projects", async (req, res) => {
    try {
      const projects = await storage.getAllBusinessProjects();
      res.json(projects);
    } catch (error: any) {
  ...
  app.post("/api/business-projects", async (req, res) => {
    try {
      const project = await storage.createBusinessProject(req.body);
      res.json(project);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.put("/api/business-projects/:id", async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const updated = await storage.updateBusinessProject(id, req.body);
  ...
  app.delete("/api/business-projects/:id", async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const deleted = await storage.deleteBusinessProject(id);

  ...
  if (selectedMission) {
    ...
    return (
      <div className="space-y-2 md:space-y-3">
        ...
        <div className="bg-card/50 rounded-lg border border-amber-900/30 p-2.5 md:p-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              {progress > 0 && (
                <div className="mt-2">
                  ...
                  <span className="text-[9px] text-muted-foreground">{progress}% complete</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className={`rounded-lg border p-2 md:p-2.5 ${styleMeta.color.replace('text-', 'border-').replace('400', '800/40')} bg-card/30`}>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[8px] text-muted-foreground ml-auto">Change in Mission Control</span>
          </div>
          <p className="text-[10px] text-muted-foreground leading-relaxed">{adaptation}</p>
        </div>

        <div>
          <h4 className="text-[10px] text-muted-foreground uppercase font-bold mb-1 md:mb-1.5 flex items-center gap-1">
        ...
        <div>
          ...
          <div className="space-y-1.5 md:space-y-2">
            {selectedMission.exercises.map((exercise) => {
              ...
              const typeMeta = EXERCISE_TYPE_META[exercise.type] || { label: exercise.type, color: 'bg-border text-muted-foreground border-border' };
              return (
                <div key={exercise.id} className={`rounded-lg border p-2 md:p-2.5 transition-colors ${isDone ? 'border-emerald-800/40 bg-emerald-900/10' : 'border-border bg-card/30'}`} data-testid={`exercise-${exercise.id}`}>
                  <div className="flex items-start gap-2">
        ...
        {selectedMission.keyTakeaways.length > 0 && (
          <div className="bg-card/30 rounded-lg border border-border p-2 md:p-2.5">
            <h4 className="text-[10px] text-muted-foreground uppercase font-bold mb-1 flex items-center gap-1">
    ...
    <div className="space-y-2 md:space-y-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5 md:gap-2">
          <div className="flex items-center gap-1 px-1.5 md:px-2 py-0.5 rounded bg-card/50 border border-border shrink-0">
          ...
          {[
            ...
            <div key={section.label} className="space-y-1.5">
              <div className="flex items-center gap-2 px-1 pt-1">
                ...
                <span className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground">{section.label}</span>
                <div className="h-px flex-1 bg-border" />
              </div>
              {section.tracks.map(track => {
                ...
                return (
                  <div key={track.id} className={`rounded-lg border transition-colors ${isExpanded ? 'border-amber-800/40 bg-card/40' : 'border-border bg-card/20'} ${!isPrereqMet ? 'opacity-60' : ''}`} data-testid={`track-${track.id}`}>
                    <button
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {trackProgress > 0 && (
                          <div className="flex items-center gap-2 mt-1">
                            <Progress value={trackProgress} className="h-0.5 flex-1 bg-border" />
                            <span className="text-[9px] text-muted-foreground">{trackProgress}%</span>
                          </div>
                        )}
                      </div>
                      {isExpanded ? <ChevronDown className="w-4 h-4 md:w-3.5 md:h-3.5 text-muted-foreground shrink-0" /> : <ChevronRight className="w-4 h-4 md:w-3.5 md:h-3.5 text-muted-foreground shrink-0" />}
                    </button>
                        ...
                        {track.missions.map(mission => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-full text-left p-2 rounded border transition-all min-h-[44px] ${
                                isActiveMission
                                  ? 'border-teal-700 bg-teal-900/20'
                                  : 'border-border hover:border-amber-800/40 active:border-amber-700/50 bg-card/50 hover:bg-card/40'
                              }`}

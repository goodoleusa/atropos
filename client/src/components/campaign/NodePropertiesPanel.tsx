    ...
    <aside className="w-72 bg-stone-950 border-l border-stone-800 p-4 space-y-4 overflow-y-auto">
      ...
      <div className="space-y-1">
        ...
        <Input
          ...
          className="bg-stone-900 border-stone-800 h-8 text-xs"
        />
      </div>
        ...
        <div className="grid grid-cols-3 gap-1">
          {NODE_TYPES.map(t => (
            <Button
              ...
              className="h-7 text-[8px] uppercase font-bold border-stone-800 gap-1"
            >
              {t.icon}{t.type}
            </Button>
          ))}
        ...
        <div className="flex gap-1 flex-wrap">
          {COLORS.map(c => (
            <button
              ...
              className={`w-6 h-6 rounded border-2 transition-all ${node.color === c ? 'scale-110 border-white' : 'border-stone-700'}`}
              style={{
      ...
      <div className="space-y-1">
        ...
        <Select value={node.pageLayout || 'card'} onValueChange={v => onUpdateNode(node.id, { pageLayout: v as any })}>
          <SelectTrigger data-testid="select-layout" className="bg-stone-900 border-stone-800 h-7 text-[10px]">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
      ...
      <div className="space-y-1.5">
        ...
        {connectedNodes.map(n => (
          <div
            key={n.id}
            className="flex items-center gap-1.5 text-[10px] text-stone-400 cursor-pointer hover:text-amber-500"
      ...
      <Button
        ...
        variant="outline"
        size="sm"
        className="w-full border-stone-800 text-stone-400 h-8 text-[10px]"
        onClick={() => {

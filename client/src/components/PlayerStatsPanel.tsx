      ...
      <DialogContent className="bg-[#0a0500] border-amber-900/50 text-stone-300 w-[calc(100vw-2rem)] max-w-lg max-h-[90vh] sm:max-h-[85vh] overflow-hidden rounded-xl">
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            <div>
              <p className="text-stone-200 font-mono font-bold">{gameState.username}</p>
            ...
            <div className="text-right">
              ...
              <p className="text-stone-600 text-[10px]">{xpForNext > 0 ? `${xpProgress}/${xpForNext} to next level` : 'Max level'}</p>
            </div>
          </div>
            ...
            {tab === 'stats' && (
              ...
              >
                ...
                {playerStats?.recentEvents?.length > 0 && (
                  <div>
                    <p className="text-[10px] text-stone-600 uppercase tracking-wider mb-2">Recent Activity</p>
                    <div className="space-y-1">
                      {playerStats.recentEvents.slice(0, 8).map((event: any, i: number) => (
                        <div key={i} className="flex items-center justify-between text-xs p-1.5 bg-stone-950/50 rounded">
                          <span className="text-stone-400">{event.eventType.replace(/_/g, ' ')}</span>
                          {event.xpAwarded > 0 && (
            ...
            {tab === 'achievements' && (
              ...
              >
                {achievementDefs.length === 0 ? (
                  <p className="text-stone-600 text-sm text-center py-8">No achievements defined yet.</p>
                ) : (
                    ...
                    return (
                      <div
                        ...
                        className={`p-3 rounded border transition-all ${
                          unlocked ? rarityStyle : 'border-stone-800 bg-stone-950/30 opacity-50'
                        }`}
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <span className={`text-sm font-mono font-bold ${unlocked ? '' : 'text-stone-600'}`}>
                          ...
                          {unlocked ? (
                            ...
                            <span className="text-[10px] text-stone-700">+{ach.xpReward} XP</span>
                          )}
                        </div>
                      </div>
                    );
            ...
            {tab === 'leaderboard' && (
              ...
              >
                {leaderboard.length === 0 ? (
                  <p className="text-stone-600 text-sm text-center py-8">No players on the leaderboard yet.</p>
                ) : (
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center gap-3 p-2.5 rounded ${
                          isMe ? 'bg-amber-900/20 border border-amber-800/50' : 'bg-stone-950/30'
                        }`}
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className={`text-sm font-mono truncate ${isMe ? 'text-amber-400' : 'text-stone-300'}`}>
                            {entry.username}
                          </p>
                          <p className="text-[10px] text-stone-600">{entry.title}</p>
                        </div>
                        <div className="text-right shrink-0">
                          <p className="text-amber-400 font-mono text-sm">{entry.xp.toLocaleString()}</p>
                          <p className="text-[10px] text-stone-600">Lv.{entry.level}</p>
                        </div>
                      </div>
                    );
                  })

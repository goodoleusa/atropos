  ...
  return (
    <section className="mb-8">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <Card className="bg-[hsl(var(--card))] border-red-900/20 hover:border-red-700/40 transition-colors shadow-sm" data-testid="quick-access-atropos">
          <CardContent className="p-4">
            ...
            <div className="space-y-1.5">
              ...
              {health?.binary?.path && (
                <div className="bg-black/40 p-1.5 rounded border border-card overflow-hidden">
                  <p className="text-[9px] text-muted-foreground font-mono truncate lowercase">{health.binary.path}</p>
                </div>
              )}
            </div>
          </CardContent>
        ...
        <Card className="bg-[hsl(var(--card))] border-teal-900/20 hover:border-teal-700/40 transition-colors shadow-sm" data-testid="quick-access-sessions">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
            ...
            <div className="flex items-end justify-between">
              ...
              <div className="flex -space-x-1">
                ...
                {activeSessions.length > 5 && (
                  <div className="w-5 h-5 rounded-full bg-card border border-border flex items-center justify-center text-[8px] text-muted-foreground font-bold">
                    +{activeSessions.length - 5}
                  </div>
                )}
              </div>
        ...
        <Card className="bg-[hsl(var(--card))] border-amber-900/20 hover:border-amber-700/40 transition-colors shadow-sm sm:col-span-2 lg:col-span-1" data-testid="quick-access-activity">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
            ...
            <div className="space-y-1.5 h-[44px] overflow-hidden">
              {(activityData?.activities || []).slice(0, 2).map((act, i) => (
                <div key={i} className="flex items-center gap-2 group">
                  ...
                  <span className="text-[10px] text-muted-foreground truncate flex-1 group-hover:text-foreground transition-colors">{act.description}</span>
              ...
              {(!activityData?.activities || activityData.activities.length === 0) && (
                <p className="text-[10px] text-muted-foreground italic">Standby - waiting for data...</p>
              )}
            </div>
          </CardContent>
        </Card>

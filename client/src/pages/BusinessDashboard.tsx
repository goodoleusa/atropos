...
function ProjectForm({
  ...
  return (
    <div className="space-y-4 p-4 bg-border/50 rounded-lg border border-border" data-testid="project-form">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">Name *</label>
          <Input data-testid="input-name" className="bg-card border-border text-foreground" value={form.name} onChange={(e) => set("name", e.target.value)} />
        </div>
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">Category</label>
          <select data-testid="select-category" className="w-full rounded-md bg-card border border-border text-foreground px-3 py-2 text-sm" value={form.category} onChange={(e) => set("category", e.target.value as Project["category"])}>
      ...
      <div>
        ...
        <Textarea data-testid="input-description" className="bg-card border-border text-foreground" value={form.description || ""} onChange={(e) => set("description", e.target.value || null)} rows={2} />
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">Stage</label>
          <select data-testid="select-stage" className="w-full rounded-md bg-card border border-border text-foreground px-3 py-2 text-sm" value={form.stage} onChange={(e) => set("stage", e.target.value as Project["stage"])}>
            {STAGES.map((s) => <option key={s} value={s}>{s}</option>)}
          </select>
        </div>
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">Status</label>
          <select data-testid="select-status" className="w-full rounded-md bg-card border border-border text-foreground px-3 py-2 text-sm" value={form.status} onChange={(e) => set("status", e.target.value as Project["status"])}>
            {STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
          </select>
        </div>
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">Priority</label>
          <select data-testid="select-priority" className="w-full rounded-md bg-card border border-border text-foreground px-3 py-2 text-sm" value={form.priority} onChange={(e) => set("priority", e.target.value as Project["priority"])}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">Budget Allocated</label>
          <Input data-testid="input-budget-allocated" type="number" className="bg-card border-border text-foreground" value={form.budget.allocated} onChange={(e) => set("budget", { ...form.budget, allocated: Number(e.target.value) })} />
        </div>
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">Budget Spent</label>
          <Input data-testid="input-budget-spent" type="number" className="bg-card border-border text-foreground" value={form.budget.spent} onChange={(e) => set("budget", { ...form.budget, spent: Number(e.target.value) })} />
        </div>
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">AI Savings</label>
          <Input data-testid="input-budget-savings" type="number" className="bg-card border-border text-foreground" value={form.budget.aiSavings} onChange={(e) => set("budget", { ...form.budget, aiSavings: Number(e.target.value) })} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div>
          ...
          <Input data-testid="input-start-date" type="date" className="bg-card border-border text-foreground" value={form.timeline.started} onChange={(e) => set("timeline", { ...form.timeline, started: e.target.value })} />
        </div>
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">Deadline</label>
          <Input data-testid="input-deadline" type="date" className="bg-card border-border text-foreground" value={form.timeline.deadline} onChange={(e) => set("timeline", { ...form.timeline, deadline: e.target.value })} />
        </div>
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">Team (Human)</label>
          <Input data-testid="input-team-human" type="number" min={0} className="bg-card border-border text-foreground" value={form.team.human} onChange={(e) => set("team", { ...form.team, human: Number(e.target.value) })} />
        </div>
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">AI Agents</label>
          <Input data-testid="input-team-ai" type="number" min={0} className="bg-card border-border text-foreground" value={form.team.aiAgents} onChange={(e) => set("team", { ...form.team, aiAgents: Number(e.target.value) })} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <Input data-testid="input-revenue" type="number" className="bg-card border-border text-foreground" value={form.revenue ?? ""} onChange={(e) => set("revenue", e.target.value ? Number(e.target.value) : null)} />
        </div>
        <div>
          <label className="text-xs text-muted-foreground mb-1 block">Notes</label>
          <Input data-testid="input-notes" className="bg-card border-border text-foreground" value={form.notes || ""} onChange={(e) => set("notes", e.target.value || null)} />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <Input data-testid="input-new-goal" className="bg-card border-border text-foreground text-sm" placeholder="Add a goal..." value={newGoal} onChange={(e) => setNewGoal(e.target.value)} onKeyDown={(e) => {
      ...
      <div className="flex gap-2 justify-end pt-2 border-t border-border">
        <Button data-testid="button-cancel" variant="ghost" className="text-muted-foreground hover:text-foreground" onClick={onCancel}>
          <X className="w-4 h-4 mr-1" /> Cancel
        </Button>
        <Button data-testid="button-save" className="bg-amber-500 text-muted-foreground hover:bg-amber-400" disabled={!form.name.trim() || saving} onClick={() => onSave(form)}>
...
function ProjectCard({
  ...
  return (
    <Card data-testid={`card-project-${project.id}`} className="bg-card/50 border-border overflow-hidden">
        ...
        {!editing && (
          <>
            <div className="flex flex-wrap gap-2 mb-3">
              ...
              <Badge className="text-xs bg-border/50 text-foreground">{project.stage}</Badge>
            </div>
            <div className="mb-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs text-muted-foreground">
              <div>
                ...
                <span className="text-foreground">${project.budget.allocated.toLocaleString()}</span>
              </div>
              <div>
                <span className="block text-muted-foreground">Spent</span>
                <span className="text-foreground">${project.budget.spent.toLocaleString()}</span>
              </div>
              <div>
                <span className="block text-muted-foreground">Team</span>
                <span className="text-foreground">{project.team.human}H + {project.team.aiAgents}AI</span>
              </div>
              <div>
                <span className="block text-muted-foreground">Deadline</span>
                <span className="text-foreground">{project.timeline.deadline}</span>
              </div>
            </div>
  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card className="bg-card/50 border-border p-6 relative overflow-hidden" data-testid="investor-arr">
          ...
          <div className="relative">
            ...
            <div className="text-xs text-muted-foreground mt-2">Target: $500K Y1 → $10M Y3</div>
          </div>
        </Card>
        <Card className="bg-card/50 border-border p-6 relative overflow-hidden" data-testid="investor-cost">
          ...
          <div className="relative">
            ...
            <div className="text-xs text-muted-foreground mt-2">vs $50,000 traditional SOC</div>
          </div>
        </Card>
        <Card className="bg-card/50 border-border p-6 relative overflow-hidden" data-testid="investor-response">
          <div className="absolute top-0 right-0 w-32 h-32 bg-amber-500/10 rounded-full blur-3xl" />
          <div className="relative">
            <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-xs text-muted-foreground mt-2">vs 4-6 hours industry avg</div>
          </div>
        </Card>
        <Card className="bg-card/50 border-border p-6 relative overflow-hidden" data-testid="investor-impact">
          ...
          <div className="relative">
            ...
            <div className="text-xs text-muted-foreground mt-2">{metrics.impact.arrestsFacilitated} arrests facilitated</div>
          </div>
        </Card>
      </div>
      ...
      <Card className="bg-card/50 border-border p-6" data-testid="investor-economics">
      ...
      <Card className="bg-card/50 border-border p-6" data-testid="investor-comparison">
      ...
      {projects.length > 0 && (
        <Card className="bg-card/50 border-border p-6" data-testid="investor-portfolio">
          <h2 className="text-2xl font-bold mb-4">Project Portfolio Overview</h2>
          <div className="space-y-3">
            {projects.slice(0, 5).map(p => (
              <div key={p.id} className="flex items-center justify-between p-3 bg-border/50 rounded-lg">
                <div className="flex items-center gap-3 min-w-0">
                  {getStageIcon(p.stage)}
                  <div className="min-w-0"><div className="text-sm font-semibold truncate">{p.name}</div><div className="text-xs text-muted-foreground capitalize">{p.stage} • {p.category}</div></div>
                </div>
                <div className="flex items-center gap-3">
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-muted-foreground text-sm">Plan, track, and pitch — all in one place</p>
          </div>
          <div className="flex bg-card border border-border rounded-lg p-1 gap-1" data-testid="view-switcher">
            <Button
              variant={topView === "business" ? "default" : "ghost"}
              size="sm"
              className={topView === "business" ? "bg-amber-500 text-muted-foreground hover:bg-amber-400" : "text-muted-foreground hover:text-foreground"}
            ...
            <Button
              ...
              size="sm"
              className={topView === "investor" ? "bg-amber-500 text-muted-foreground hover:bg-amber-400" : "text-muted-foreground hover:text-foreground"}
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="projects" className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <Button data-testid="button-add-project" className="bg-amber-500 text-muted-foreground hover:bg-amber-400" onClick={() => setShowNewForm(!showNewForm)}>
            ...
            {isLoading && <div className="text-center py-12 text-muted-foreground">Loading projects...</div>}
            ...
            {!isLoading && projects.length === 0 && !showNewForm && (
              <Card className="bg-card/50 border-border p-12 text-center">
                ...
                <Button data-testid="button-add-first-project" className="bg-amber-500 text-muted-foreground hover:bg-amber-400" onClick={() => setShowNewForm(true)}>
          ...
          <TabsContent value="planning" className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
              {STAGES.map((stage) => {
                ...
                return (
                  <div key={stage} className="bg-card/30 rounded-lg border border-border p-3" data-testid={`column-${stage}`}>
                    ...
                    <div className="space-y-2">
                      {stageProjects.map((p) => (
                        <Card key={p.id} className="bg-border/50 border-border p-3" data-testid={`kanban-card-${p.id}`}>
                          ...
                          <div className="text-[10px] text-muted-foreground mt-1">{p.progress}%</div>
                        </Card>
                      ))}
                      {stageProjects.length === 0 && (
                        <div className="text-xs text-muted-foreground text-center py-4">No projects</div>
                      )}
                    </div>
                  </div>
                );
              ...
              <div className="space-y-3">
                ...
                {[...projects]
                  ...
                  .map((p) => {
                    ...
                    return (
                      <div key={p.id} className="flex items-center justify-between p-3 bg-border/50 rounded-lg" data-testid={`timeline-project-${p.id}`}>
                        <div className="flex items-center gap-3 min-w-0">
                          {getStageIcon(p.stage)}
                          <div className="min-w-0">
